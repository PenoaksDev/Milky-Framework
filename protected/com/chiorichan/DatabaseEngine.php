<?php
	class DatabaseEngine
	{
		...
		public function query($query, $debug = false, $pdo = CONFIG_SITE)
		{
			...
			GetFramework()->getServer()->Debug1("ChioriDB: Manual Query: " . $query);
			...
			{
				$error = $pdo->errorInfo();
				GetFramework()->getServer()->Warning("ChioriDB: Making query \"" . $query . "\" which returned with error: \"" . $error[2] . "\".");
				return array();
			}
		...
		public function SQLInjectionDetection($QueryString)
		{
			...
			if (!$QuerySafe)
				GetFramework()->getServer()->Panic(400, "SQL Injection Detected! Notify administrators ASAP. Debug \"" . $QueryString . "\".");
		...
		function update($table, $data, $where = "", $limit = 0, $pdo = CONFIG_SITE, $DisableInjectionCheck = false) // $data accepts JSON String and Array.
		{
			...
			if ($result !== false)
			{
				GetFramework()->getServer()->Debug1("ChioriDB: Making UPDATE query \"" . $query . "\" which affected " . $result . " rows.");
				return true;
			}
			else
			{
				$error = $pdo->errorInfo();
				GetFramework()->getServer()->Debug1("ChioriDB: Making UPDATE query \"" . $query . "\" which had no affect on the database, Error: " . $error[2] . ".");
				return false;
			}
		}
		...
		public function delete($table, $where = "", $limit = 0, $pdo = CONFIG_SITE)
		{
			...
			$this->SQLInjectionDetection($query);
			GetFramework()->getServer()->Debug1("ChioriDB: " . $query);
			
			$result = $pdo->exec($query);
			if ($result !== false)
			{
				GetFramework()->getServer()->Debug1("ChioriDB: Making DELETE query \"" . $query . "\" which affected " . $result . " rows.");
				return $result;
			}
			else
			{
				$error = $pdo->errorInfo();
				GetFramework()->getServer()->Debug1("ChioriDB: Making DELETE query \"" . $query . "\" which had no affect on the database, Error: " . $error[2] . ".");
				return false;
			}
		}
		...
		function insert($table, $data, $pdo = CONFIG_SITE, $DisableInjectionCheck = false) // $data accepts JSON String and Array.
		{
			...
			if ($result !== false)
			{
				GetFramework()->getServer()->Debug1("ChioriDB: Making INSERT query \"" . $query . "\" which affected " . $result . " rows.");
				return true;
			}
			else
			{
				$error = $pdo->errorInfo();
				GetFramework()->getServer()->Debug1("ChioriDB: Making INSERT query \"" . $query . "\" which had no affect on the database, Error: \"" . $error[2] . "\".");
				return false;
			}
		}
