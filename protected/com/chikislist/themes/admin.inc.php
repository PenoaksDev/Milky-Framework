...
<script type="text/javascript">
	...
	function loadNew(page, self)
	{
		...
		page = "http://panel.chikislist.com/" + page;
	...
	{
//		console.log("Loading AJAX Page: http://panel.chikislist.com/" + url);
		$("div.tooltip").hide();
		
		var self = $('#ajaxframe');
		
		self.fadeOut(300, function () {
			self.load("http://panel.chikislist.com/" + url, function () {
				self.fadeIn(300);
			});
		});
	...
	function goto_frame_post(url, xml) //old
	{
		...
		$('#ajaxframe').fadeOut(300, function () {
			$.ajax({
				url: "http://panel.chikislist.com/" + url,
				type: "post",
				data: {xml: escape(xml)},
				success:
					function (data) {
	...
	{
		// console.log("Loading AJAX Page: http://www.chikislist.com/dashboard/" + url);
		$("div.tooltip").hide();
	...
	$(document).ready(function()
	{
		...
		<?
			$opt = $chiori->com->chiorichan->modules->settings->get("USER_HOMEPAGE", "", true);
			if (!empty($opt))
			{
				echo("goto_frame(\"" . $opt . "\");");
			}
...
<? $chiori->nameSpaceInclude("com.chikislist.widget.adminheader"); ?>

<div id="wrapper" class="div01" style="margin-bottom: 20px;">
	<div id="buffer"></div>
	<div id="body">
		<div id="whole" class="a-box">
			<div id="ajaxframe">
				<!-- PAGE DATA -->
			</div>
			<!-- INFO -->
