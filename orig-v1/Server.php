...
class Server
{
	...
	private $default_session_name = "NorthShore";
	private $data = [];
	private $domain;
	...
	{
		// TODO: Temp
		$this->serverName = "Framework Server";
	}
	...
	function Debug( $msg )
	{
		$this->rawData( $msg, LOG_DEBUG );
	}

	function Debug1( $msg )
	{
		$this->rawData( $msg, LOG_DEBUG1 );
	}

	function Debug2( $msg )
	{
		$this->rawData( $msg, LOG_DEBUG2 );
	}

	function Debug3( $msg )
	{
		$this->rawData( $msg, LOG_DEBUG3 );
	}

	function Info( $msg )
	{
		$this->rawData( $msg, LOG_INFO );
	}

	function Notice( $msg )
	{
		$this->rawData( $msg, LOG_NOTICE );
	}

	function Warning( $msg )
	{
		$this->rawData( $msg, LOG_WARNING );
	}

	function Error( $msg )
	{
		$this->rawData( $msg, LOG_ERR );
	}

	function Critical( $msg )
	{
		$this->rawData( $msg, LOG_CRIT );
	}

	function Alert( $msg )
	{
		$this->rawData( $msg, LOG_ALERT );
	}

	function Emergency( $msg )
	{
		$this->rawData( $msg, LOG_EMERG );
	}
	...
	public function rawData( $message, $level = LOG_DEBUG )
	{
		...
		$bt = debug_backtrace();
		$bt = $bt[1];
		$arr = [date( "h:i:" ) . $this->getseconds(), basename( $bt["file"] ), $bt["line"], $level];
