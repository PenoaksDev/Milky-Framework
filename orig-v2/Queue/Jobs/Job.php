				...
				if (is_array($d))
{
	 ...
	 *
	 * @param  mixed  $value
	 * @return \Penoaks\Contracts\Queue\QueueableEntity
	 */
		...
		if (method_exists($this->instance, 'failed'))
{
	 ...
	 *
	 * @return \Penoaks\Contracts\Queue\EntityResolver
	 */
	protected function getEntityResolver()
	{
		return $this->bindings->make('Penoaks\Contracts\Queue\EntityResolver');
	}
		...
		if ($delay instanceof DateTime)
{
	...
	public function resolveName()
	{
		...
		if ($name === 'Penoaks\Queue\CallQueuedHandler@call')
{
			return Arr::get($payload, 'data.commandName', $name);
		}

		if ($name === 'Penoaks\Events\CallQueuedHandler@call')
{
