...
class Util
{
	...
	public static function createCalendarFromEvents( Collection $events )
	{
		...
		{
			$calendarEvents[] = Calendar::event( $event->title, $event->all_day, $event->starts, $event->ends, $event->id, [
					'url' => $event->url,
					'color' => '#f44336'
				] );
		}
	...
	public static function rand( $length, $numbers = true, $letters = true, $allowedChars = [] )
	{
		if ( $allowedChars == null )
			$allowedChars = [];

		if ( $numbers )
			$allowedChars = array_merge( $allowedChars, ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"] );

		if ( $letters )
			$allowedChars = array_merge( $allowedChars, [
				"A",
				"B",
				"C",
				"D",
