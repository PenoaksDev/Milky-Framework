...
class Dispatcher implements DispatcherContract
{
	 ...
	 *
	 * @param  \Penoaks\Contracts\Broadcasting\ShouldBroadcast $event
	 * @return void
	 */
	protected function broadcastEvent( $event )
	{
		if ( $this->queueResolver )
		{
			...
			/* connection() seems to be implemented in the QueueManager class, why is it being called on a Queue instance? */
			$this->resolveQueue()->connection( $connection )->pushOn( $queue, 'Penoaks\Broadcasting\BroadcastEvent', [
	...
	public function createClassListener( $listener )
	{
		...
		{
			return call_user_func_array( $this->createClassCallable( $listener, $bindings ), func_get_args() );
		};
	}
	...
	protected function handlerShouldBeQueued( $class )
	{
		try
		{
			return ( new ReflectionClass( $class ) )->implementsInterface( 'Penoaks\Contracts\Queue\ShouldQueue' );
		}
		catch ( Exception $e )
		{
			return false;
	...
	protected function createQueuedHandlerCallable( $class, $method )
	{
		return function () use ( $class, $method )
		{
			...
			{
				$this->resolveQueue()->push( 'Penoaks\Events\CallQueuedHandler@call', [
					'class' => $class,
					'method' => $method,
	...
	protected function callQueueMethodOnHandler( $class, $method, $arguments )
	{
		...
		$handler->queue( $this->resolveQueue(), 'Penoaks\Events\CallQueuedHandler@call', [
			'class' => $class,
			'method' => $method,
			'data' => serialize( $arguments ),
		] );
