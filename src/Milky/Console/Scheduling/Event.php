...
class Event
{
	...
	protected function callBeforeCallbacks()
	{
		foreach ( $this->beforeCallbacks as $callback )
			UniversalBuilder::call( $callback );
	}
	...
	{
		if ( !$this->runsInMaintenanceMode() && Framework::fw()->isDownForMaintenance() )
			return false;

		return $this->expressionPasses() && $this->runsInEnvironment( Framework::fw()->environment() );
	}
	...
	public function filtersPass()
	{
		foreach ( $this->filters as $callback )
			if ( !UniversalBuilder::call( $callback ) )
				return false;
	...
	{
		return $this->spliceIntoPosition( 1, 0 )->spliceIntoPosition( 2, 0 )->spliceIntoPosition( 3, 1 )->spliceIntoPosition( 4, '*/3' );
	}
